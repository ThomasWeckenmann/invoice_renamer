...
- **Milestone 3 — done.** Detailed empirical results, per-model speed/accuracy data, and debugging lessons live in `docs/model_benchmark_findings.md` (a findings log, not a plan) — read that before re-deriving anything here.
  ...
  - **Start here next**: Milestone 4 (desktop workflow) — BB-11 file transaction/Undo (Tauri/Rust: atomic preflight, rename, persistent Undo record).
...
- **Milestone 4 — BB-10 done (React batch workspace); BB-11 not started.** `app/src/lib/api/` is a hand-authored TypeScript mirror of the worker's Pydantic contracts (no OpenAPI codegen exists yet, so this isn't the `types/generated/` the repo-structure sketch describes) plus a fetch client that resolves the session endpoint via the existing `get_worker_endpoint` Tauri command. `app/src/features/batch/` covers PDF import (file picker + HTML5 drag-and-drop, reading bytes directly — no new Tauri plugin), model selection grouped Open/Local vs Closed/Cloud with live download/remove controls and progress polling (`useModelCatalog`), job submission with per-item status polling (`useBatchWorkspace`), an editable-filename review list with extracted fields/warnings, and per-item/approve-all approval. 17 Vitest tests pass; `npm run lint` and `npm run build` (tsc + vite) are clean.
  - **Known gap, deliberate**: import only captures file bytes, not real filesystem paths — a plain `<input type=file>`/HTML5-drop File never exposes one. BB-11 needs real source paths to rename originals, which means getting them requires BB-01's native-dialog capability (`@tauri-apps/plugin-dialog`'s path-returning `open()`, plus the webview drag-drop event API for drop) — not yet wired, since that's Tauri/Rust work outside BB-10's own React/TypeScript ownership. The "Rename approved" action is therefore a disabled button with an inline note today; wiring it up is what BB-11 (plus this path-capture piece of BB-01) needs to unblock.
    - Found live on macOS during manual testing: HTML5 drag-and-drop onto `ImportDropzone` doesn't fire with real files at all — Tauri intercepts native OS drag-drop by default (`dragDropEnabled`), so the DOM `drop` event never gets a populated `dataTransfer`. The click-to-browse file picker still works via the plain `<input type=file>`. Switching drag-and-drop to `@tauri-apps/api/webview`'s `onDragDropEvent` fixes both this and the path-capture gap above in one move, since that API hands back real paths directly — do that as part of the BB-11/BB-01 path-capture work rather than patching drag-and-drop separately now.
